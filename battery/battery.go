...
func GetAttrAsString(ctx *types.Context, attr string, round bool, nth int) (string, error) {
	...
	status := ""
	if !ok {
		return status, fmt.Errorf("attr %s not recognized", attr)
	}

	fieldValue := reflect.ValueOf(*batt).FieldByName(fieldName).Interface()

	switch value := fieldValue.(type) {
	case float64:
		if round {
			status = fmt.Sprintf("%d", int(math.Round(value)))
		} else {
			status = fmt.Sprintf("%f", value)
		}
	case string:
		status = value
	default:
