...
func TestMaybeReadINI(t *testing.T) {
	t.Parallel()
	{
		release, err := reader.MaybeReadINI("../testdata/etc/os-release")
		assert.NoError(t, err)
		assert.Equal(t, "18.04", release.Version)
	}
	{
		// if the file does not exist on this system, that's not an error
		release, err := reader.MaybeReadINI("../testdata/etc/os-releasezzz")
		assert.NoError(t, err)
		assert.Nil(t, release)
	}
	{
		// if the file cannot be parsed, that's an error
		release, err := reader.MaybeReadINI("../testdata/etc/not-an-ini-file")
		assert.Error(t, err)
		assert.Nil(t, release)
	}
}
